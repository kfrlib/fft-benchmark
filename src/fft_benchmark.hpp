...
inline size_t product(const std::vector<size_t>& sizes)
{
...
template <size_t dims>
inline size_t product(const std::array<size_t, dims>& sizes)
{
    size_t result = sizes[0];
    for (size_t i = 1; i < sizes.size(); ++i)
        result *= sizes[i];
...
inline std::string sizes_to_string(const std::vector<size_t>& sizes)
{
    std::string result;
    for (size_t n : sizes)
    {
...
fft_impl_ptr<real> fft_create_for(const sizes_t<dims>& size, real* out, const real* in,
                                  std::integral_constant<bool, is_complex>,
                                  std::integral_constant<bool, invert>, std::integral_constant<bool, inplace>)
{
    ...
    {
        return fft_impl_ptr<real>(
            new fft_implementation<dims, real, is_complex, invert, inplace>(size, out, in));
    }
    return nullptr;
}
...
{
    if (inplace)
        return fft_create_for<fft_implementation, real>(
            size, out, in, std::integral_constant<bool, is_complex>{}, std::integral_constant<bool, invert>{},
...
template <template <int, typename, bool, bool, bool> typename fft_implementation, typename real, size_t dims,
          bool is_complex>
fft_impl_ptr<real> fft_create_for(const sizes_t<dims>& size, real* out, const real* in,
                                  std::integral_constant<bool, is_complex>, bool invert, bool inplace)
{
    if (invert)
        return fft_create_for<fft_implementation, real>(size, out, in,
                                                        std::integral_constant<bool, is_complex>{},
...
template <template <int, typename, bool, bool, bool> typename fft_implementation, typename real, size_t dims>
fft_impl_ptr<real> fft_create_for(const sizes_t<dims>& size, real* out, const real* in, bool is_complex,
                                  bool invert, bool inplace)
{
    if (is_complex)
        return fft_create_for<fft_implementation, real>(size, out, in, std::integral_constant<bool, true>{},
                                                        invert, inplace);
    else
        return fft_create_for<fft_implementation, real>(size, out, in, std::integral_constant<bool, false>{},
                                                        invert, inplace);
}

template <template <int, typename, bool, bool, bool> typename fft_implementation, typename real>
fft_impl_ptr<real> fft_create_for(const std::vector<size_t>& size, real* out, const real* in, bool is_complex,
                                  bool invert, bool inplace)
{
    if (size.size() == 1)
        return fft_create_for<fft_implementation, real>(sizes_t<1>{ size[0] }, out, in, is_complex, invert,
                                                        inplace);
    if (size.size() == 2)
        return fft_create_for<fft_implementation, real>(sizes_t<2>{ size[0], size[1] }, out, in, is_complex,
                                                        invert, inplace);
    if (size.size() == 3)
        return fft_create_for<fft_implementation, real>(sizes_t<3>{ size[0], size[1], size[2] }, out, in,
                                                        is_complex, invert, inplace);
    return nullptr;
}

extern template fft_impl_ptr<float> fft_create<float>(const std::vector<size_t>& size, float* out,
                                                      const float* in, bool is_complex, bool inverse,
                                                      bool inplace);
extern template fft_impl_ptr<double> fft_create<double>(const std::vector<size_t>& size, double* out,
                                                        const double* in, bool is_complex, bool inverse,
                                                        bool inplace);
